...
def validar_id(id):
    ...
    try:
        ...
        raise ValueError(f"El ID '{id}' no es un número válido")
    if id_ <= 0:
        raise ValueError(f"El ID '{id_}' debe ser mayor a 0")
    return id_
...
def validar_fecha(fecha_str):
    ...
    if not patron.match(fecha_str):
        raise ValueError(f"La fecha '{fecha_str}' no tiene el formato YYYY-MM-DD")
    return fecha_str
